...
Description:
    Configure Meshtastic SX1302 concentrator daemon and forward packets to a internet as a long range sink!
...
#include "bridge/MeshtasticBridge.h"   // Unified bridge C API
#include "bridge/MeshtasticSX126xShim.h"

/* Meshtastic protocol hooks backed by SX126x-style runtime shim */
#ifdef __cplusplus
extern "C" {
#endif
    extern void* meshtastic_init_and_setup(void);
    extern void meshtastic_run_loop(void);
    extern int meshtastic_send_data(uint8_t topic, const char* message, int length, const uint8_t* targetDevice);
#ifdef __cplusplus
}
#endif
...
int mqtt_message_arrived(void *context, char *topicName, int topicLen, MQTTClient_message *message) {
    ...
    if (message->payloadlen > 0 && message->payload != NULL) {
        // Parse JSON command format: {"target":"NODE","topic":20,"message":"LED_ON"}
        ...
        int result = meshtastic_send_data(topic, cmd_message, cmd_len, target_device);
        
        // Send acknowledgment response
        char response[512];
        int response_len;
        if (result == 0) {
            MSG("[MQTT] Command forwarded to Meshtastic bridge network successfully\n");
            response_len = snprintf(response, sizeof(response),
                "{\"status\":\"success\",\"target\":\"%.8s\",\"topic\":%d,\"message\":\"%s\"}",
                target_device, topic, cmd_message);
        } else {
            MSG("ERROR: [MQTT] Failed to forward command to Meshtastic bridge network: %d\n", result);
            response_len = snprintf(response, sizeof(response),
...
void thread_duck(void);  /* Meshtastic runtime processing */
...
int main(int argc, char ** argv)
{
    ...
    pthread_t thrid_jit;
    pthread_t thrid_ss;
    pthread_t thrid_duck;  /* Meshtastic processing thread */
    ...
    MSG("INFO: [main] Initializing Meshtastic protocol\n");
    void* hub_ptr = meshtastic_init_and_setup();
    if (hub_ptr == NULL) {
        MSG("ERROR: [main] failed to initialize Meshtastic protocol\n");
        exit(EXIT_FAILURE);
    }
    MSG("INFO: [main] Meshtastic protocol initialized successfully\n");
    ...
    if (i != 0) {
        MSG("ERROR: [main] impossible to create Meshtastic processing thread\n");
        exit(EXIT_FAILURE);
    }
	...
	*/
    ...
    MSG("INFO: Exiting meshtasticd program\n");
    exit(EXIT_SUCCESS);
}
...
void thread_down(void) {
    ...
    /* Zaihan's code */
    /* Buffer for a possible downlink from Meshtastic */
    ...
    while (!exit_sig && !quit_sig) {
            ...
            /* Try to pop a downlink from Meshtastic */
            int duck_rc = mtk_bridge_pop_downlink(duck_payload_buf, &buf_capacity,
                                                   &dl_freq_hz, &dl_tmst, &dl_tx_power_dbm,
                                                   &dl_bw_hz, &dl_sf, &dl_cr, &dl_rf_chain);

        if (duck_rc != 1) {
            MSG("DEBUG: [thread_down] mtk_bridge_pop_downlink returned %d, buf_capacity=%u\n", duck_rc, buf_capacity);
        }
        ...
        while (((int)difftimespec(recv_time, send_time) < keepalive_time) && !exit_sig && !quit_sig) {
            ...
            /* Check for Meshtastic downlinks inside inner loop for faster response */
            int duck_rc_inner = mtk_bridge_pop_downlink(duck_payload_buf, &buf_capacity,
                                                         &dl_freq_hz, &dl_tmst, &dl_tx_power_dbm,
                                                         &dl_bw_hz, &dl_sf, &dl_cr, &dl_rf_chain);
            ...
            if (duck_rc_inner == 0 && buf_capacity > 0) {
                ...
                MSG("INFO: Meshtastic downlink TX freq=%u Hz\n", txpkt.freq_hz);
                
                /* Always get fresh timestamp - Meshtastic packets are always IMMEDIATE */
                pthread_mutex_lock(&mx_concent);
                lgw_get_instcnt(&current_concentrator_time);
                pthread_mutex_unlock(&mx_concent);
                ...
                jit_result = jit_enqueue(&jit_queue[txpkt.rf_chain], current_concentrator_time, &txpkt, downlink_type);
                if (jit_result == JIT_ERROR_OK) {
                    MSG("INFO: Meshtastic packet enqueued to JIT queue\n");
                    pthread_mutex_lock(&mx_meas_dw);
                    meas_nb_tx_requested += 1;
                    pthread_mutex_unlock(&mx_meas_dw);
                } else {
                    MSG("ERROR: Meshtastic packet rejected by JIT queue: %d\n", jit_result);
                }
...
/* -------------------------------------------------------------------------- */
/* --- THREAD: MESHTASTIC PROTOCOL PROCESSING ------------------------------ */

void thread_duck(void) {
    MSG("INFO: Meshtastic processing thread started\n");
    
    /* Give other threads time to initialize */
    wait_ms(1000);
    MSG("INFO: Meshtastic runtime thread starting main loop\n");
    
    /* Counter for periodic MQTT health check (every 30 seconds) */
    int mqtt_check_counter = 0;
    
    /* Main loop: call hub.main() to process Meshtastic packets */
    while (!exit_sig && !quit_sig) {
        /* Call the Meshtastic main loop to process received packets */
        meshtastic_run_loop();
